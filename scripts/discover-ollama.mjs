...
const DEFAULT_MODEL = process.env.VITE_OLLAMA_MODEL || process.env.OLLAMA_MODEL || 'qwen2.5:7b'
...
export async function discoverOllama() {
  try {
    ...
    const qwen = names.find((n) => /qwen/i.test(n))
    const model = qwen || names[0] || DEFAULT_MODEL
